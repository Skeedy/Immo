<?php
$url = _ROOT_ADMIN.'?controller='.$_controller.'&view=list&annoncessortby='.$_renovation_sort;
...
if(!isAjax()) {
    ...
    <div class="title clearfix">
        <h1 class="pull-left">Toutes les renovations</h1>
    ...
    <form method="get">
        <div class="table-responsive">
            <table id="annonceslist" class="table table-list table-striped">
                <thead>
                <tr>
                    <th class="hassort minimum"><?php printSortList('annonces', 'Ordre ', 'ordre', $_renovation_sort, $url.'&', '#annonceslist'); ?></th>
                    <th>Image</th>
                    <th class="hassort minimum"><?php printSortList('annonces', 'ID ', 'id', $_renovation_sort, $url.'&', '#annonceslist'); ?></th>
                    <th class="hassort"><?php printSortList('annonces', 'Ref', 'ref', $_renovation_sort, $url.'&', '#annonceslist'); ?></th>
                    <th class="hassort"><?php printSortList('annonces', 'Date', 'date', $_renovation_sort, $url.'&', '#annonceslist'); ?></th>
                    <th class="hassort"><?php printSortList('annonces', 'Titre', 'titre', $_renovation_sort, $url.'&', '#annonceslist'); ?></th>
                    <th class="hassort"><?php printSortList('annonces', 'Ville', 'nom_reel', $_renovation_sort, $url.'&', '#annonceslist'); ?></th>
                    <th class="hassort minimum text-center"><?php printSortList('annonces', 'Publié', 'active', $_renovation_sort, $url.'&', '#annonceslist'); ?></th>
                    <th>Actions</th>
                </tr>
                <tr class="filter">
                    <th></th>
                    <th>
                        <input type="hidden" name="controller" value="renovations">
                        <input type="hidden" name="view" value="list">
                        <input type="hidden" name="annoncessortby" value="<?php echo $_renovation_sort; ?>">
                    </th>
                    <th>
                ...
                if(!empty($_renovation->annonces)) {
                    foreach($_renovation->annonces as $v) {
                        $images = !empty($v->images) && json_decode($v->images) ? json_decode($v->images) : array();
                        $comparaisons = !empty($v->comparaisons) && json_decode($v->comparaisons) ? json_decode($v->comparaisons) :
                            array();
                        ...
                        <tr id="annonce_<?php echo $v->id; ?>">
                            ...
                            <td class="minimum">
                                ...
                                if(!empty($images)) {
                                    ?>
                                    <a class="multiline" style="position:relative" href="<?php echo _ROOT_ADMIN.'?controller='.$_controller; ?>&view=renovation&renovation=<?php echo $v->id; ?>">
                                        <img src="<?php echo _ROOT.$db_renovation->getRenovationDirImg($v->id).'sm_'
                                            .$images[0]->image; ?>" alt="">
                            ...
                            <td>
                                <a class="multiline" href="<?php echo _ROOT_ADMIN.'?controller='.$_controller;
                                ?>&view=renovation&renovation=<?php echo $v->id; ?>">
                                    <?php echo $v->id; ?>
                                </a>
                            </td>
                            <td>
                                <a class="multiline nowrap" href="<?php echo _ROOT_ADMIN.'?controller='.$_controller; ?>&view=renovation&renovation=<?php echo $v->id; ?>">
                                    <?php echo $v->ref; ?>
                                </a>
                            </td>
                            <td class="minimum text-center">
                            ...
                            <td>
                                <a class="multiline" href="<?php echo _ROOT_ADMIN.'?controller='.$_controller; ?>&view=renovation&renovation=<?php echo $v->id; ?>">
                                    <?php echo $v->titre;?>
                            ...
                            <td class="text-center">
                                ...
                                &nbsp;
                                <a class="btn btn-primary btn-sm" title="Détails" href="<?php echo _ROOT_ADMIN.'?controller='.$_controller; ?>&view=renovation&renovation=<?php echo $v->id; ?>"><span class="glyphicon glyphicon-edit"></span></a>
                                <a class="btn btn-danger btn-sm" title="Supprimer" href="<?php echo _ROOT_ADMIN.'?controller='.$_controller; ?>&view=list&urlreturn=<?php echo urlencode($url_current); ?>&cancel_url=<?php echo urlencode($url_current); ?>&action=delete_renovation&renovation=<?php echo $v->id; ?>"><span class="glyphicon glyphicon-trash"></span></a>
                            </td>
                        </tr>
                        <?php
                    }
