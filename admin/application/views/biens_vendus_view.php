<?php
$url = _ROOT_ADMIN.'?controller='.$_controller.'&view=vendus&annoncessortby='.$_annonces_sort;
...
<div class="title clearfix">
	<h1 class="pull-left">Tous les biens vendus</h1>
...
<table id="annonceslist" class="table table-list table-striped">
	<thead>
		<tr>
			...
			<th class="hassort"><?php printSortList('annonces', 'Titre ('._LANG_DEFAULT.')', 'titre', $_annonces_sort, $url.'&', '#annonceslist'); ?></th>
		...
		<tr class="filter">
			...
			<th>
				...
				<input type="hidden" name="view" value="vendus">
			...
			<th class="minimum">
				...
				<a class="btn btn-sm btn-danger" href="<?php echo _ROOT_ADMIN.'?controller='.$_controller.'&view=vendus'; ?>">Reset</button>
			</th>
		</tr>
	</thead>
	<tbody>
			...
			<tr id="annonce_<?php echo $v->id; ?>">
				...
				<td>
					<a class="multiline" href="<?php echo _ROOT_ADMIN.'?controller='.$_controller; ?>&view=bien&bien=<?php echo $v->id; ?>">
						...
						<?php if(isset($v->pieces)) echo ' - '.$v->pieces.' pièce'.($v->pieces > 1 ? 's' : ''); ?>
					</a><br>
				...
				<td class="text-center">
					...
					<a class="btn btn-danger btn-sm" title="Supprimer" href="<?php echo _ROOT_ADMIN.'?controller='.$_controller; ?>&view=vendus&urlreturn=<?php echo urlencode($url_current); ?>&cancel_url=<?php echo urlencode($url_current); ?>&action=delete_bien&bien=<?php echo $v->id; ?>"><span class="glyphicon glyphicon-trash"></span></a>
				</td>
			</tr>
			<?php
		}
...
$(function() {
	...
	var annonces_ids = new Bloodhound({
		...
		remote: {
			url: "<?php echo _ROOT_ADMIN.'?controller='.$_controller; ?>&vendus=1&searchannonces_ids=",
	...
	var annonces_refs = new Bloodhound({
		...
		queryTokenizer: Bloodhound.tokenizers.whitespace,
		remote: {
			url: "<?php echo _ROOT_ADMIN.'?controller='.$_controller; ?>&vendus=1&searchannonces_refs=",
			prepare: function(query, settings) {
				settings.url = settings.url += encodeURIComponent(query);
				return settings;
			}
	...
	var titres = new Bloodhound({
		...
		queryTokenizer: Bloodhound.tokenizers.whitespace,
		remote: {
			url: "<?php echo _ROOT_ADMIN.'?controller='.$_controller; ?>&vendus=1&searchtitres=",
			prepare: function(query, settings) {
				settings.url = settings.url += encodeURIComponent(query);
				return settings;
			}
	...
	var annoncesvilles = new Bloodhound({
		...
		queryTokenizer: Bloodhound.tokenizers.whitespace,
		remote: {
			url: "<?php echo _ROOT_ADMIN.'?controller='.$_controller; ?>&vendus=1&searchannoncesvilles=",
			prepare: function(query, settings) {
				settings.url = settings.url += encodeURIComponent(query);
				return settings;
			}
