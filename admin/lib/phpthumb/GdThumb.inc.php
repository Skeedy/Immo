...
class GdThumb extends ThumbBase
{
	...
	{
		if(!($watermarkfile = imagecreatefrompng($watermark)))
		{
			throw new InvalidArgumentException ('invalid watermark file');
		}
		...
		imagecopy(
			$this->oldImage,
			$watermarkfile,
			$this->currentDimensions['width'] / 2 - imagesx($watermarkfile) / 2,
			$this->currentDimensions['height'] / 2 - imagesy($watermarkfile) / 2,
			0,
			0,
			imagesx($watermarkfile),
			imagesy($watermarkfile)
		);
	...
	{
		$newWidthPercentage	= $this->maxWidth / $width;
		$newHeight		= $height * $newWidthPercentage;
	...
	{
		$newHeightPercentage	= $this->maxHeight / $height;
		$newWidth 		= $width * $newHeightPercentage;
