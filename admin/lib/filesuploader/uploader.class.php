...
class FileUpload {
    ...
    public $sizeLimit = 20971520;         // Max file upload size in bytes (default 10MB)
    ...
    private function errorCodeToMsg($code) {
        switch($code) {
            case UPLOAD_ERR_INI_SIZE:
                $message = 'Le fichier dépasse la limite de '.($this->sizeLimit / 1024 / 1024).'Mo';
                break;
            case UPLOAD_ERR_PARTIAL:
                $message = 'Le fichier a été partiellement uploadé.';
                break;
            ...
            case UPLOAD_ERR_CANT_WRITE:
                $message = 'L\'écriture sur le disque a échoué.';
                break;
    ...
    public function handleUpload($uploadDir = null, $allowedExtensions = null) {
        if (!$this->fileName) {
            $this->setErrorMsg('Nom incorrect ou aucun upload');
            return false;
        }

        if ($this->fileSize == 0) {
            $this->setErrorMsg('Le fichier est vide');
            return false;
        }

        if ($this->fileSize > $this->sizeLimit) {
            $this->setErrorMsg('Le fichier dépasse la limite de '.($this->sizeLimit / 1024 / 1024).'Mo');
            return false;
        }
        ...
        if (!is_writable($this->uploadDir)) {
            $this->setErrorMsg('Impossible d\'écrire dans le dossier');
            return false;
        }
        ...
        if (!empty($this->allowedExtensions)) {
            if (!$this->checkExtension($this->fileExtension, $this->allowedExtensions)) {
                $this->setErrorMsg('Type de fichier non autorisé');
                return false;
            }
        }
        ...
        if (!$this->save($this->savedFile)) {
            $this->setErrorMsg('Le fichier n\'a pas pu être sauvegardé');
            return false;
        }
